...
rule SEPP:
    ...
    shell:
        """ 
        qiime fragment-insertion sepp --i-representative-sequences {input.qiime_fasta} --i-reference-database {params.ref} --o-tree {output.tree} --o-placements {output.placements} --p-threads 30
        """
rule export_tree:
    input:
